...
import "./strategies/Common/StrategyCommonSolidlyRewardPool.sol";
import "./vaults/BeefyVaultV7Factory.sol";

import "@openzeppelin-5/contracts/token/ERC20/ERC20.sol";

contract AetosSwapxSolidlyDeployer {
    string public constant STRATEGY_NAME = "aetos-solidly-2";
    ...
    address public constant SWAPX_ROUTER = address(0xF5F7231073b3B41c04BA655e1a7438b1a7b29c27);
    ...
    event Deployed(string name, string symbol, address vault, address strategy, address lp);
    ...
    function deploy(
        ...
        StrategyCommonSolidlyRewardPool strategy = StrategyCommonSolidlyRewardPool(payable(strategyFactory.createStrategy(STRATEGY_NAME)));
        ...
        ISolidlyRouter.Routes[] memory path = new ISolidlyRouter.Routes[](0);

        vault.initialize(IStrategyV7(address(strategy)), name, symbol, 0);
        strategy.initialize(gauge, SWAPX_ROUTER, path, path, false, rewards, addresses);
        _deposit(vault, want, amount);

        emit Deployed(name, symbol, address(vault), address(strategy), want);
    }
